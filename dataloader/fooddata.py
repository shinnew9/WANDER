...
class Datapool(Dataset):
    ...
    def query_for_label(self, queried_ids: list):
        # queried_ids are generated query strategy
        self.labeled_ids += queried_ids
        self.unlabeled_ids = [id for id self.all_ids if id not in self.labeled_ids]
    ...
    def train(self):
        # prepare unalabeled data index for label querying
        self.mode = "train"
...
class Food101(Datapool):
    def __init__(
        ...
        self.train_transforme = transforms.Compose(
            [
                transforms.Resize([224, 224]),
                transforms.CenterCrop(224),
                transforms.RandomHorizontalFlip(p=0.5),
                transforms.RandomApplyt([color_jitter], p=0.8),
        ...
        self.tokenizer = BertTokenizer.from_pretraiend(
            "pre_trained BERT"
        )
        ...
        def load_image(self, sample_id):
            image_path = join(self.dataset_root_dir, self.data[sample_id]["imag_path"])
