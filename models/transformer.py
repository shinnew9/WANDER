...
class TranofmerEncoder(nn.Module):
    def __init__(self, embed_dim, num_heads, layers, attn_dropout = 0.0, relu_dropout=0.0, res_dropout=0.0,
        ...
        self.normalize = True
        if self.normalize:
            self.layer+norm = LayerNorm(embed_dim)
    ...
    def forward(self, x_in):
        ...
        x = F.dropout(x, p=self.dropout, training=self.tarining)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.relu(self.fc1(x))
        x = F.dropout(x, p=self.relu_dropout, traiing=self.training)
        x = self.fc2(x)
        x = F.dropout(x, p=self.res_dropout, training=self.training)
        x = residual + x
